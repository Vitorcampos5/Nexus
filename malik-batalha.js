  ...
  function iniciarConfrontoMalik(cenario) {
    ...
    function oNexusResiste() {
      ...
      function esferasChegam() {
        ...
        var raioFinal = Math.max(rect.width, rect.height) / 2 + 26;
        for (var i = 0; i < 7; i++) {
          (function (i) {
            var ang = (i / 7) * Math.PI * 2;
            var destX = cx + Math.cos(ang) * raioFinal, destY = cy + Math.sin(ang) * raioFinal;
            var origX = cx + Math.cos(ang) * (raioFinal + 260), origY = cy + Math.sin(ang) * (raioFinal + 260);
            var orb = el('div', 'position:fixed;width:16px;height:16px;border-radius:50%;background:radial-gradient(circle at 35% 30%,#FFD98A,#E8891A 62%,#A34E08);box-shadow:0 0 8px rgba(232,137,26,.7);left:' + origX + 'px;top:' + origY + 'px;transition:left 1.4s cubic-bezier(.2,.8,.2,1),top 1.4s cubic-bezier(.2,.8,.2,1);z-index:700010;pointer-events:none;');
      ...
      daquiA(3400, esferasChegam);
      daquiA(5400, brocaSeSolta);
      daquiA(6400, mensagemPersonifica);
      daquiA(9000, leynAparece);
      daquiA(10600, observacaoEGolpe);
      daquiA(12500, malikExpulso);
      daquiA(14200, restauracaoCompleta);
    ...
    var DURACOES = [0,    8000,      8000,       7000,       3200,         4500,             0];

    function avancarPara(n) {
      faseAtual = n;
      if (FASES[n]) FASES[n]();
      if (n < 6 && DURACOES[n]) daquiA(DURACOES[n], function () { avancarPara(n + 1); });
    }
